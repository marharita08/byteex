...
interface Props {
  data: SectionPartnersAndProperties;
}
...
const GalleryContent = ({
  ...
  galleryImages: { main: string; thumb: string; name: string }[];
...
export const PartnersAndProperties = ({ data }: Props) => {
  const { asSeenInTitle, partnersLabels, title, properties, galleryItems } =
    data.fields;
                    ...
                    <div className="flex flex-col gap-1.5 pt-1">
                      ...
                      {prop.desc && (
                        <p className="text-[14px] md:text-[15px] font-suisse text-neutral-300 leading-[22px] md:leading-[23px] tracking-[0.03em] max-w-[340px] md:max-w-none">
                          {documentToReactComponents(prop.desc)}
                        </p>
                      )}
                    </div>
                  </div>
                  {i < propertiesList.length - 1 && (
